  ...
  const handleDelete = async (id, nome) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        alert(data.detail || "Erro ao deletar usuário");
      }
    } catch (error) {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input 
                required
                type="text" 
                className="w-full border rounded p-2 text-sm focus:ring-2 focus:ring-brand-blue outline-none"
            ...
            <button type="submit" className="w-full bg-brand-blue text-white font-bold py-2 rounded hover:bg-blue-700 transition flex items-center justify-center gap-2">
              Cadastrar
            </button>
          </form>
        </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {usuarios.map((user) => (
                <tr key={user.id} className="hover:bg-gray-50 transition">
                  <td className="p-4 font-medium text-gray-700">{user.nome}</td>
                  <td className="p-4 text-gray-500 text-sm">{user.email}</td>
                  <td className="p-4">
                    <span className={`text-xs px-2 py-1 rounded-full font-bold ${user.cargo === 'admin' ? 'bg-purple-100 text-purple-600' : 'bg-green-100 text-green-600'}`}>
                      {user.cargo}
                    </span>
                  </td>
                  {/* Botão Deletar */}
